/**
 * @preserve FullCalendar v1.5.2 http://arshaw.com/fullcalendar/
 * 
...
(function($, undefined) {
	...
	$.fn.fullCalendar = function(options) {
		...
		// would like to have this logic in EventManager, but needs to happen
		// before options are recursively extended
	...
	function Calendar(element, options, eventSources) {
		...
		function initialRender() {
			...
			// needed for IE in a 0x0 iframe, b/c when it is resized, never
			// triggers a windowResize
			if (!bodyVisible()) {
				lateRender();
			}
		}
		...
		function lateRender() {
			setTimeout(function() { // IE7 needs this so dimensions are
		...
		// TODO: improve view switching (still weird transition in IE, and FF
		// has whiteout problem)
		function changeView(newViewName) {
			if (!currentView || newViewName != currentView.name) {
				ignoreWindowResize++; // because setMinHeight might change the
										// height before render (and
										// subsequently setSize) is reached
				...
				if (oldView) {
					(oldView.beforeHide || noop)(); // called before changing
													// min-height. if called
													// after, scroll state is
													// reset (in Opera)
					setMinHeight(content, content.height());
					oldView.element.hide();
				} else {
					setMinHeight(content, 1); // needs to be 1 (not 0) for
												// IE7, or else view dimensions
												// miscalculated
				}
				...
				if (currentView) {
					...
					currentView = viewInstances[newViewName] = new fcViews[newViewName](
							newViewElement = absoluteViewElement = $(
									...
									.appendTo(content), t // the calendar
															// object
					);
				}
				...
				renderView(); // after height has been set, will make
								// absoluteViewElement's position=relative, then
								// set to null
		...
		function renderView(inc) {
			if (elementVisible()) {
				ignoreWindowResize++; // because renderEvents might
										// temporarily change the height before
										// setSize is reached
				...
				if (!currentView.start || inc || date < currentView.start
					...
					// view must render an entire new date range (and
					// refetch/render events)
					currentView.render(date, inc || 0); // responsible for
														// clearing events
					setSize(true);
		...
		function windowResize() {
			if (!ignoreWindowResize) {
				if (currentView.start) { // view has already been rendered
					...
					// calendar must have been initialized in a 0x0 iframe that
		...
		// fetches events if necessary, rerenders events if necessary (or if
		// forced)
		...
		function select(start, end, allDay) {
			currentView
					.select(start, end, allDay === undefined ? true : allDay);
		}
	...
	function Header(calendar, options) {
															...
															if (buttonClick) {
																...
																var text = smartProperty(
																		...
																		buttonName); // why
																						...
																						// smartProperty
																						// here?
																var button = $("<span class='fc-button fc-button-"
																		+ buttonName
																		+ " "
																		+ tm
																		+ "-state-default'>"
		...
		function activateButton(buttonName) {
			element.find('span.fc-button-' + buttonName).addClass(
					tm + '-state-active');
		}

		function deactivateButton(buttonName) {
			element.find('span.fc-button-' + buttonName).removeClass(
					tm + '-state-active');
		}
	...
	function EventManager(options, _sources) {
		...
		function _fetchEventSource(source, callback) {
			...
			if (events) {
				if ($.isFunction(events)) {
					pushLoading();
					events(cloneDate(rangeStart), cloneDate(rangeEnd),
							function(events) {
								callback(events);
								popLoading();
							});
		...
		function addEventSource(source) {
			...
			if (source) {
				pendingSourceCnt++;
				fetchEventSource(source, currentFetchID); // will eventually
															// call reportEvents
			}
		}
		...
		function updateEvent(event) { // update an existing event
			...
			startDelta = event.start - event._start, endDelta = event.end ? (event.end - (event._end || defaultEventEnd(event))) // event._end
																																	// would
																																	// be
																																	// null
																																	// if
		...
		function removeEvents(filter) {
			if (!filter) { // remove all
				...
				for (var i = 0; i < sources.length; i++) {
					if ($.isArray(sources[i].events)) {
						sources[i].events = $.grep(sources[i].events, filter,
								true);
					}
				}
			}
		...
		function normalizeEvent(event) {
			...
			// TODO: if there is no start date, return false to indicate an
			// invalid event
		}
	...
	function zeroDate() { // returns a Date with time 00:00:00 and
							// dateOfMonth=1
		var i = 0, d;
		do {
			d = new Date(1970, i++, 1);
	...
	function parseDate(s, ignoreTimezone) { // ignoreTimezone defaults to true
		...
		// TODO: never return invalid dates (like from new Date(<string>)),
		// return null instead
		return null;
	}

	function parseISO8601(s, ignoreTimezone) { // ignoreTimezone defaults to
												// false
	...
	function hsides(element, includeMargins) {
		return hpadding(element) + hborders(element)
	...
	function vsides(element, includeMargins) {
		return vpadding(element) + vborders(element)
	...
	function setMinHeight(element, height) {
		...
		element.each(function(i, _element) {
			_element.style.cssText += ';min-height:' + height + ';_height:'
					+ height;
	...
	function disableTextSelection(element) {
		element.attr('unselectable', 'on').css('MozUserSelect', 'none').bind(
	...
	/*
	 * function enableTextSelection(element) { element .attr('unselectable',
	 * 'off') .css('MozUserSelect', '') .unbind('selectstart.ui'); }
	 */

	function markFirstLast(e) {
		e.children().removeClass('fc-first fc-last').filter(':first-child')
	...
	function getSkinCss(event, opt) {
		...
		var backgroundColor = event.backgroundColor || eventColor
				|| source.backgroundColor || sourceColor
				|| opt('eventBackgroundColor') || optionColor;
		var borderColor = event.borderColor || eventColor || source.borderColor
				|| sourceColor || opt('eventBorderColor') || optionColor;
		var textColor = event.textColor || source.textColor
				|| opt('eventTextColor');
		var statements = [];
		if (backgroundColor) {
	...
	function BasicWeekView(element, calendar) {
		...
		function render(date, delta) {
			...
			t.title = formatDates(visStart, addDays(cloneDate(visEnd), -1),
					opt('titleFormat'));
			t.start = start;
			t.end = end;
			t.visStart = visStart;
	...
	function BasicView(element, calendar, viewName) {
		...
		function buildSkeleton(maxRowCnt, showNumbers) {
			...
			s = "<table class='fc-border-separate' style='width:100%' cellspacing='0'>"
					+ "<thead>" + "<tr>";
			for (i = 0; i < colCnt; i++) {
				s += "<th class='fc- " + headerClass + "'/>"; // need fc- for
																// setDayID
			}
			s += "</tr>" + "</thead>" + "<tbody>";
		...
		function setHeight(height) {
			...
			if (opt('weekMode') == 'variable') {
				rowHeight = rowHeightLast = Math.floor(bodyHeight
						/ (rowCnt == 1 ? 2 : 6));
			} else {
		...
		function renderDayOverlay(overlayStart, overlayEnd,
				refreshCoordinateGrid) { // overlayEnd is exclusive
			...
			for (var i = 0; i < rowCnt; i++) {
				...
				if (stretchStart < stretchEnd) {
					var colStart, colEnd;
					if (rtl) {
						colStart = dayDiff(stretchEnd, rowStart) * dis + dit
								+ 1;
						colEnd = dayDiff(stretchStart, rowStart) * dis + dit
								+ 1;
					} else {
	...
	function BasicEventRenderer() {
		...
		function compileSegs(events) {
			var rowCnt = getRowCnt(), colCnt = getColCnt(), d1 = cloneDate(t.visStart), d2 = addDays(
					cloneDate(d1), colCnt), visEventsEnds = $.map(events,
					exclEndDay), i, row, j, level, k, seg, segs = [];
			for (i = 0; i < rowCnt; i++) {
...
eventElement.draggable({
				...
				start : function(ev, ui) {
					...
					hoverListener.start(function(cell, origCell, rowDelta,
							colDelta) {
						...
						if (cell) {
							...
							dayDelta = rowDelta * 7 + colDelta
									* (opt('isRTL') ? -1 : 1);
							renderDayOverlay(addDays(cloneDate(event.start),
									dayDelta), addDays(exclEndDay(event),
									dayDelta));
						} else {
				...
				stop : function(ev, ui) {
					...
					if (dayDelta) {
						eventDrop(this, event, dayDelta, 0, event.allDay, ev,
								ui);
					} else {
	...
	function AgendaView(element, calendar, viewName) {
		...
		function buildSkeleton() {
			...
			s = "<table style='width:100%' class='fc-agenda-days fc-border-separate' cellspacing='0'>"
					+ "<thead>"
					+ "<tr>"
					+ "<th class='fc-agenda-axis "
					+ headerClass + "'>&nbsp;</th>";
			for (i = 0; i < colCnt; i++) {
				s += "<th class='fc- fc-col" + i + ' ' + headerClass + "'/>"; // fc-
																				// needed
																				// for
																				// setDayID
			}
			s += "<th class='fc-agenda-gutter " + headerClass + "'>&nbsp;</th>"
					...
					+ "'>&nbsp;</th>";
			for (i = 0; i < colCnt; i++) {
				s += "<td class='fc- fc-col" + i + ' ' + contentClass + "'>" + // fc-
																				// needed
																				// for
																				// setDayID
				"<div>" + "<div class='fc-day-content'>"
						+ "<div style='position:relative'>&nbsp;</div>"
						+ "</div>" + "</div>" + "</td>";
			}
			s += "<td class='fc-agenda-gutter " + contentClass
					+ "'>&nbsp;</td>" + "</tr>" + "</tbody>" + "</table>";
			...
			if (opt('allDaySlot')) {
				...
				axisFirstCells = axisFirstCells.add(allDayTable
						.find('th:first'));
				gutterCells = gutterCells.add(allDayTable
						.find('th.fc-agenda-gutter'));

				slotLayer.append("<div class='fc-agenda-divider " + headerClass
		...
		function setHeight(height, dateChanged) {
			...
			dayBodyFirstCellStretcher.height(bodyHeight
		...
		function setWidth(width) {
			...
			var slotTableWidth = slotScroller[0].clientWidth; // needs to be
																// done after
																// axisWidth
																// (for IE7)
		...
		function resetScroll() {
			...
			scroll();
			setTimeout(scroll, 0); // overrides any previous scroll state made
									// by the browser
		}
		...
		function slotClick(ev) {
			if (!opt('selectable')) { // if selectable, SelectionManager will
				...
				var col = Math.min(colCnt - 1, Math.floor((ev.pageX
						- dayTable.offset().left - axisWidth)
						/ colWidth));
		...
		function renderCellOverlay(row0, col0, row1, col1) { // only for
																// all-day?
		...
		coordinateGrid = new CoordinateGrid(function(rows, cols) {
			...
			var slotScrollerBottom = slotScrollerTop
					+ slotScroller.outerHeight();
			function constrain(n) {
				return Math.max(slotScrollerTop, Math
						.min(slotScrollerBottom, n));
			}
			for (var i = 0; i < slotCnt; i++) {
				rows.push([ constrain(slotTableTop + slotHeight * i),
		...
		// objects)
		function timePosition(day, time) { // both date objects. day holds
			...
			var slotMinutes = opt('slotMinutes'), minutes = time.getHours()
					* 60 + time.getMinutes() - minMinute, slotI = Math
					.floor(minutes / slotMinutes), slotTop = slotTopCache[slotI];
			if (slotTop === undefined) {
				slotTop = slotTopCache[slotI] = slotTable.find('tr:eq(' + slotI
						+ ') td div')[0].offsetTop; // .position().top; // need
													// this optimization???
			}
			return Math.max(0, Math.round(slotTop - 1 + slotHeight
		...
		function renderSelection(startDate, endDate, allDay) { // only for
																// all-day
			if (allDay) {
				if (opt('allDaySlot')) {
		...
		function renderSlotSelection(startDate, endDate) {
			...
			if (helperOption) {
				...
				if (col >= 0 && col < colCnt) { // only works when times are on
												// same day
					var rect = coordinateGrid.rect(0, col, 0, col, slotContent); // only
																					// for
																					// horizontal
					...
					if (bottom > top) { // protect against selections that are
										// entirely before or after visible
										// range
						rect.top = top;
						rect.height = bottom - top;
						rect.left += 2;
						...
						if ($.isFunction(helperOption)) {
							...
							if (helperRes) {
								...
								rect.zIndex = 8;
								selectionHelper = $(helperRes).css(rect)
										.appendTo(slotContent);
							}
						} else {
		...
		function slotSelectionMousedown(ev) {
			if (ev.which == 1 && opt('selectable')) { // ev.which==1 means
														// left mouse button
				unselect(ev);
				var dates;
				hoverListener
		...
		function reportDayClick(date, allDay, ev) {
			trigger('dayClick', dayBodyCells[dayOfWeekCol(date.getDay())],
					date, allDay, ev);
		}
			...
			hoverListener.start(function(cell) {
				...
				if (cell) {
					if (cellIsAllDay(cell)) {
						renderCellOverlay(cell.row, cell.col, cell.row,
								cell.col);
					} else {
		...
		function dragStop(_dragElement, ev, ui) {
			...
			clearOverlays();
			if (cell) {
				trigger('drop', _dragElement, cellDate(cell),
						cellIsAllDay(cell), ev, ui);
			}
		}
	...
	function AgendaEventRenderer() {
		...
		var renderDaySegs = t.renderDaySegs;
		var resizableDayEvent = t.resizableDayEvent; // TODO: streamline
		...
		function compileDaySegs(events) {
			var levels = stackSegs(sliceSegs(events, $.map(events, exclEndDay),
					t.visStart, t.visEnd)), i, levelCnt = levels.length, level, j, seg, segs = [];
		...
		function compileSlotSegs(events) {
			var colCnt = getColCnt(), minMinute = getMinMinute(), maxMinute = getMaxMinute(), d = addMinutes(
					cloneDate(t.visStart), minMinute), visEventEnds = $.map(
					events, slotEventEnd), i, col, j, level, k, seg, segs = [];
			for (i = 0; i < colCnt; i++) {
				col = stackSegs(sliceSegs(events, visEventEnds, d, addMinutes(
		...
		function renderSlotSegs(segs, modifiedEventId) {
			...
			for (i = 0; i < segCnt; i++) {
				...
				if (eventElement = seg.element) {
					...
					seg.vsides = val === undefined ? (vsideCache[key] = vsides(
							eventElement, true)) : val;
					val = hsideCache[key];
					seg.hsides = val === undefined ? (hsideCache[key] = hsides(
							eventElement, true)) : val;
			...
			for (i = 0; i < segCnt; i++) {
				...
				if (eventElement = seg.element) {
					...
					event = seg.event;
					if (seg.contentTop !== undefined
							&& height - seg.contentTop < 10) {
						// not enough room for title, put it in the time header
						eventElement.find('div.fc-event-time').text(
		...
		function slotSegHtml(event, seg) {
			...
			html += " class='"
					...
					+ htmlEscape(formatDates(event.start, event.end,
							opt('timeFormat'))) + "</div>" + "</div>"
					+ "<div class='fc-event-content'>"
					+ "<div class='fc-event-title'>" + htmlEscape(event.title)
...
eventElement.draggable({
						zIndex : 9,
						opacity : opt('dragOpacity', 'month'), // use whatever
																// the month
																// view was
																// using
						revertDuration : opt('dragRevertDuration'),
						start : function(ev, ui) {
							trigger('eventDragStart', eventElement, event, ev,
									ui);
							hideEvents(event, eventElement);
							origWidth = eventElement.width();
							hoverListener
									.start(
											function(cell, origCell, rowDelta,
													colDelta) {
												clearOverlays();
												if (cell) {
													// setOverflowHidden(true);
						...
						stop : function(ev, ui) {
							hoverListener.stop();
							clearOverlays();
							trigger('eventDragStop', eventElement, event, ev,
									ui);
							if (revert) {
								// hasn't moved or is out of bounds (draggable
								...
								var minuteDelta = 0;
								if (!allDay) {
									minuteDelta = Math.round((eventElement
											.offset().top - getBodyContent()
											.offset().top)
											/ slotHeight)
											* opt('slotMinutes')
											+ minMinute
											- (event.start.getHours() * 60 + event.start
													.getMinutes());
								}
								eventDrop(this, event, dayDelta, minuteDelta,
										allDay, ev, ui);
							}
							// setOverflowHidden(false);
						}
			...
			function resetElement() {
				if (!allDay) {
					eventElement.width(origWidth).height('').draggable(
							'option', 'grid', null);
					allDay = true;
				}
			}
...
eventElement.draggable({
					...
					hoverListener.start(function(cell, origCell, rowDelta,
						...
						if (cell) {
							...
							if (opt('allDaySlot') && !cell.row) {
								...
								renderDayOverlay(addDays(
										cloneDate(event.start), dayDelta),
				...
				stop : function(ev, ui) {
					...
					if (cell && (dayDelta || minuteDelta || allDay)) {
						// changed!
						eventDrop(this, event, dayDelta, allDay ? 0
								: minuteDelta, allDay, ev, ui);
					} else {
						// either no change or out-of-bounds (draggable has
						// already reverted)
						resetElement();
						eventElement.css('filter', ''); // clear IE opacity
														// side-effects
						eventElement.css(origPosition); // sometimes fast drags
														// make event revert to
														// wrong position
						updateTimeText(0);
						showEvents(event, eventElement);
					}
...
eventElement.resizable({
				...
				resize : function(ev, ui) {
					...
					// account
					slotDelta = Math.round((Math.max(slotHeight, eventElement
							.height()) - ui.originalSize.height)
							/ slotHeight);
					if (slotDelta != prevSlotDelta) {
						timeElement.text(formatDates(event.start,
								(!slotDelta && !event.end) ? null : // no
																	...
																	// time
																	// range
								addMinutes(eventEnd(event), opt('slotMinutes')
										* slotDelta), opt('timeFormat')));
				...
				stop : function(ev, ui) {
					...
					if (slotDelta) {
						...
						showEvents(event, eventElement);
						// BUG: if event was really short, need to put title
						// back in span
					}
				}
			});
	...
	function View(element, calendar, viewName) {
		...
		function trigger(name, thisObj) {
			return calendar.trigger.apply(calendar, [ name, thisObj || t ]
		...
		function eventDrop(e, event, dayDelta, minuteDelta, allDay, ev, ui) {
			...
			trigger('eventDrop', e, event, dayDelta, minuteDelta, allDay,
					function() {
						...
						// might not work
						moveEvents(eventsByID[eventId], -dayDelta,
	...
	function DayEventRenderer() {
		...
		function renderTempDaySegs(segs, adjustRow, adjustTop) {
			...
			var segCnt = segs.length;
			var element;
			tempContainer[0].innerHTML = daySegHTML(segs); // faster than
															// .html()
		...
		function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
			...
			for (i = 0; i < segCnt; i++) {
				...
				html += " class='" + classes.join(' ') + "'"
						+ " style='position:absolute;z-index:8;left:" + left
						+ "px;" + skinCss + "'" + ">" + "<div"
				...
				if (seg.isEnd && isEventResizable(event)) {
					html += "<div class='ui-resizable-handle ui-resizable-"
							+ (rtl ? 'w' : 'e') + "'>" + "&nbsp;&nbsp;&nbsp;" + // makes
																				// hit
																				// area
																				// a
		...
		function daySegHandlers(segs, segmentContainer, modifiedEventId) {
			...
			var element;
			var event;
			// retrieve elements, run through eventRender callback, bind
			// handlers
			for (i = 0; i < segCnt; i++) {
				seg = segs[i];
				element = seg.element;
		...
		function daySegSetWidths(segs) {
			...
			for (i = 0; i < segCnt; i++) {
				...
				element = seg.element;
				if (element) {
					element[0].style.width = Math.max(0, seg.outerWidth
							- seg.hsides)
							+ 'px';
				}
			}
		...
		function getRowDivs() {
			...
			for (i = 0; i < rowCnt; i++) {
				rowDivs[i] = allDayRow(i).find(
						'td:first div.fc-day-content > div'); // optimal
																// selector?
			}
			return rowDivs;
		}
		...
		function daySegSetTops(segs, rowTops) { // also triggers
												// eventAfterRender
			var i;
			var segCnt = segs.length;
			var seg;
		...
		function resizableDayEvent(event, element, seg) {
			...
			disableTextSelection(element); // prevent native <a> selection for
											// IE
			element.mousedown(function(ev) { // prevent native <a> selection
												// for others
				ev.preventDefault();
			}).click(function(ev) {
				if (isResizing) {
			...
			handle.mousedown(function(ev) {
				...
				clearSelection();
				$('body').css('cursor', direction + '-resize').one('mouseup',
						mouseup);
				...
				hoverListener.start(
						function(cell, origCell) {
							if (cell) {
								...
								var c = cell.col;
								if (rowCnt == 1) {
									r = 0; // hack for all-day area in agenda
											// views
								}
								if (r == minCell.row) {
									if (rtl) {
								...
								dayDelta = (r * 7 + c * dis + dit)
										- (origCell.row * 7 + origCell.col
												* dis + dit);
								var newEnd = addDays(eventEnd(event), dayDelta,
										true);
								if (dayDelta) {
								...
								clearOverlays();
								renderDayOverlay(event.start, addDays(
										cloneDate(newEnd), 1)); // coordinate
																// grid already
																// rebuild at
																// hoverListener.start
							}
				...
				function mouseup(ev) {
					...
					// otherwise, the drag handler already restored the old
					// events

					setTimeout(function() { // make this happen after the
	...
	function SelectionManager() {
		...
		function daySelectionMousedown(ev) { // not really a generic manager
			...
			var reportDayClick = t.reportDayClick; // this is hacky and sort of
													// weird
			if (ev.which == 1 && opt('selectable')) { // which==1 means left
														// mouse button
				unselect(ev);
				var _mousedownElement = this;
				var dates;
				hoverListener.start(function(cell, origCell) { // TODO: maybe
																// put
																// cellDate/cellIsAllDay
																// info in cell
					clearSelection();
					if (cell && cellIsAllDay(cell)) {
						dates = [ cellDate(origCell), cellDate(cell) ]
								.sort(cmp);
	...
	function CoordinateGrid(buildFunc) {
		...
		t.rect = function(row0, col0, row1, col1, originElement) { // row1,col1
																	// is
																	// inclusive
		...
		function mouse(ev) {
			...
			if (!newCell != !cell || newCell
				...
				if (newCell) {
					...
					change(newCell, firstCell, newCell.row - firstCell.row,
	...
	function HorizontalPositionCache(getElement) {
		...
		t.left = function(i) {
			return lefts[i] = lefts[i] === undefined ? e(i).position().left
					: lefts[i];
		};

		t.right = function(i) {
			return rights[i] = rights[i] === undefined ? t.left(i)
					+ e(i).width() : rights[i];
		};
...
PrimeFaces.widget.Schedule = function(cfg) {
	...
	if (this.jq.is(':not(:visible)')) {
		var hiddenParent = this.jq.parents('.ui-hidden-container:first'), hiddenParentWidget = hiddenParent
				.data('widget');
...
PrimeFaces.widget.Schedule.prototype.setupEventHandlers = function() {
	...
	this.cfg.eventDrop = function(calEvent, dayDelta, minuteDelta, allDay,
	...
	this.cfg.eventResize = function(calEvent, dayDelta, minuteDelta,
			revertFunc, jsEvent, ui, view) {
		if (_self.cfg.behaviors) {
...
PrimeFaces.widget.Schedule.prototype.setupEventSource = function() {
	var _self = this, offset = new Date().getTimezoneOffset() * 60000
			+ this.cfg.offset;
...
var options = {
			...
			onsuccess : function(responseXML) {
				var xmlDoc = $(responseXML.documentElement), updates = xmlDoc
						.find("update");

				for (var i = 0; i < updates.length; i++) {
					var update = updates.eq(i), id = update.attr('id'), data = update
							.text();
					...
					if (id == _self.id) {
						...
						PrimeFaces.ajax.AjaxUtils.updateElement.call(this, id,
								data);
					}
				}
